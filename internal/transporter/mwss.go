...
// 	if err != nil {
// 		logger.Logger.Infof("[mwss server err] %s - %s : %s", conn.RemoteAddr(), s.Addr(), err)
// 		return
// 	}
// 	defer mux.Close()

// 	logger.Logger.Infof("[mwss server init] %s  %s", conn.RemoteAddr(), s.Addr())
// 	defer logger.Logger.Infof("[mwss server close] %s >-< %s", conn.RemoteAddr(), s.Addr())
...
// 	}
// 	defer wsc.Close()
// 	logger.Logger.Infof("handleTcpOverMWSS from:%s to:%s", c.RemoteAddr(), wsc.RemoteAddr())
// 	if err := transport(wsc, c); err != nil {
// 		return err
// 	}
// 	return nil
// }
...
// 	}
// 	defer rc.Close()
// 	logger.Logger.Infof("handleMWSSConnToTcp from:%s to:%s", c.RemoteAddr(), rc.RemoteAddr())
// 	if err := transport(rc, c); err != nil {
// 		logger.Logger.Infof("transport err: %s", err.Error())
// 	}
// }
