...
## Engineering Discipline: SSOT & Community Libraries

- **Single Source of Truth (SSOT)**: `*config.Config` is the sole authority for runtime configuration across all subsystems. Never duplicate config fields or maintain parallel state mirrors. Metrics and traffic statistics must flow through single authoritative trackers (`meteredOutbound`, `connTracker`).
- **Lean on Community Libraries, No Reinvented Wheels**: Go ecosystem provides robust, production-proven packages (`golang.org/x/...`, `echo`, `xray-core`, `urfave/cli`). Never implement ad-hoc protocol framing, custom crypto routines, or homemade network buffers.
